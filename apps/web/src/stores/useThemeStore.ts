...
type AppliedTheme = ResolvedTheme | 'white' | 'wool';
...
const isTheme = (theme: unknown): theme is Theme => {
  return theme === 'auto' || theme === 'white' || theme === 'wool' || theme === 'dark';
};
...
const applyTheme = (resolved: AppliedTheme) => {
  ...
  if (resolved === 'wool') {
    document.documentElement.setAttribute('data-theme', 'wool');
    return;
  }
  ...
  persist(
    (set, get) => ({
      ...
      cycleTheme: () => {
        ...
        const order: Theme[] = ['auto', 'white', 'wool', 'dark'];
